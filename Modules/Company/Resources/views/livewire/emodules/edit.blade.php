<div class="w-full p-1 md:p-4">
    <x-lf.card title="Edit" class="warning">
		...
		<x-lf.form.input name="icon" type="string" label="Icon" placeholder="Icon ..."/>
        ...
        <x-slot name="tools" >
            ...
            <a class="btn-success sm" href="{{route('company.emodules.show',$record_id)}}">{!! lfIcon("launch",11) !!}</a>
            @endcan
            <a class="btn-primary sm" href="{{route('company.emodules')}}">{!! lfIcon("list",11) !!}</a>
        </x-slot>
        <x-slot name="footer">
            <div class="card-footer flex justify-between">
                <label class="btn-primary flex-none" wire:click="store">Update</label>
                <a class="btn" href="{{route("company.emodules")}}">Cancel</a>
            </div>
        </x-slot>
    </x-lf.card>
</div>
