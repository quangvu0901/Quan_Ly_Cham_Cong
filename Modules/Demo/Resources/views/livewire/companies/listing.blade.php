<x-lf.page.listing :fields="$fields" :footer="$data->onEachSide(3)->links()">
        ...
        @foreach($data as $item)
            <tr>
                ...
                <td class="action">
                    ...
                    <a class="btn-success xs" href="{{route("demo.companies.show",$item->id)}}">{!! lfIcon("launch",10) !!}</a>
                    @endcan
                    @can('demo.companies.edit')
                    <a class="btn-info xs" href="{{route("demo.companies.edit",$item->id)}}">{!! lfIcon("edit",10) !!}</a>
                    @endcan
                    @can('demo.companies.delete')
    ...
    <x-slot name="tools">
        @can("demo.companies.create")
           <div> <a class="btn-primary sm" href="{{route("demo.companies.create")}}">{!! lfIcon("add") !!}</a></div>
        @endcan
    </x-slot>
</x-lf.page.listing>
