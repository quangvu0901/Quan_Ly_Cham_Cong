<div class="w-full p-1 md:p-4">
    <x-lf.card title="Edit" class="warning">
        <x-lf.form.input name="name" type="string" label="Tên chấm công" placeholder="Tên luật chấm công ..." onClick="this.select();"/>
        <x-lf.form.toggle name="active" type="boolean" label="Đạt làm kiểu chấm công mặc định" onClick="this.select();"/>
        <div style="width: 100%">
            @if ($type == 1)
                <div style=" ">
...
<x-slot name="tools">
      <a class="btn-primary sm" href="{{route('time-keep.timekeeprules')}}">{!! lfIcon("list",11) !!}</a>
</x-slot>
<x-slot name="footer">
