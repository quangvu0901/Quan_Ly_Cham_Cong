...
class   Create extends Component
{
    ...
    public $name, $value = [], $status = 1,$option = [],$day_apply = [],$type,$active = 0,
       ...
       $option_penance_after = [],$option_penance_before = [],$hour_shift;
...
//        'option_after_morning.*.time_after'=>'date_format:"H:i"|after:input_morning|before:output_morning',
        'option_after_morning.*.count_after'=>'min:0|max:4',
...
//        'option_after_noon.*.time_after'=>'date_format:"H:i"|after:input_noon|before:output_noon',
//        'option_before_noon.*.time_before'=>'date_format:"H:i"|after:"13:00"|before:output_noon',
        'option_before_noon.*.count_before'=>'numeric|max:count_noon',
        //Ca
...
//        'option_after_shift.*.time_after'=>'date_format:"H:i"|after:input_shift|before:output_shift',
        'option_after_shift.*.count_after'=>'max:count_shift',
//        'option_before_shift.*.time_before'=>'date_format:"H:i"|after:input_shift|before:output_shift',
        'option_before_shift.*.count_before'=>'max:count_shift',
        'day_apply'=>'required',
    ...
    public function mount()
    {
        ...
        array_push($this->option_after_shift,["time_after"=>"","count_after"=>""]);
        array_push($this->option_before_shift,["time_before"=>"","count_before"=>""]);
        array_push($this->option_penance_after,["time_penance_after"=>"","money_penance_after"=>""]);
        array_push($this->option_penance_before,["time_penance_before"=>"","money_penance_before"=>""]);

        $this->authorize("time-keep.timekeeprules.create");
        $this->done = 1;
    }
    ...
    public function updatedCountMorning(){
        ...
        if ($this->count_morning){
            ...
            $this->validate([
                'count_morning' => 'required|numeric|between:0,4'
            ]);
        }
    }
    ...
    public function updatedOptionAfterMorning()
    {
        foreach ($this->option_after_morning as $key => $option)
            $callback = function (Validator $validator) {
                foreach ($this->option_after_morning as $key => $option)
                    $pass = $validator->passes();
                if (!$pass){
                    $this->option_after_morning[$key]['time_after'] = '';
                }
            };
        if ($option['time_after'] != null){
            $this->withValidatorCallback = $callback;
            $this->validate([
                'option_after_morning.*.time_after' => 'date_format:"H:i"|after:input_morning|before:output_morning',
            ],[
        ...
        $callback_count_after = function (Validator $validator) {
            foreach ($this->option_after_morning as $key => $option)
                $pass = $validator->passes();
            if (!$pass){
                $this->option_after_morning[$key]['count_after'] = '';
            }
        };
        if ($this->input_morning == null && $this->output_morning == null && $this->count_morning == null &&
            $option['time_after'] != null || $option['count_after'] != null) {
            $this->withValidatorCallback = $callback_count_after;
            $this->validate([
                'input_morning' => 'required',
                'output_morning' => 'required',
                'count_morning' => 'required',
//                'option_after_morning.*.time_after' => 'required',
            ]);
        }
        if ($this->input_morning != null && $this->output_morning != null && $this->count_morning != null && $option['time_after'] != null ){
            $this->withValidatorCallback = $callback_count_after;
            $this->validate([
                'option_after_morning.*.count_after' => '|numeric|between:0,4'
            ],[
        ...
        foreach ($this->option_before_morning as $key => $option)
            $callback = function (Validator $validator) {
                foreach ($this->option_before_morning as $key => $option)
                    $pass = $validator->passes();
                if (!$pass){
                    $this->option_before_morning[$key]['time_before'] = '';
                }
            };
        if ($option['time_before'] != null){
            $this->withValidatorCallback = $callback;
            $this->validate([
                'option_before_morning.*.time_before' => 'date_format:"H:i"|after:input_morning|before:output_morning',
            ],[
...
//                'before' => 'thời gian nhập phải trước giờ tan  làm ca chiều'
        ...
        $callback_count_before = function (Validator $validator) {
            foreach ($this->option_before_morning as $key => $option)
                $pass = $validator->passes();
            if (!$pass){
                $this->option_before_morning[$key]['count_before'] = '';
            }
        };
        if ($this->input_morning == null && $this->output_morning == null && $this->count_morning == null &&
            $option['time_before'] != null || $option['count_before'] != null /*&& $option['time_after_morning'] == null*/) {
            $this->withValidatorCallback = $callback_count_before;
            $this->validate([
                'input_morning' => 'required',
                'output_morning' => 'required',
                'count_morning' => 'required',
//                'option_before_morning.*.time_before' => 'required',
//                'option_after_morning.*.time_after' => 'required',
                'option_after_morning.*.count_after' => 'required',
            ]);
        }
        if ($this->input_morning != null && $this->output_morning != null && $this->count_morning != null && $option['time_before'] != null ){
            $this->withValidatorCallback = $callback_count_before;
            $this->validate([
                'option_before_morning.*.count_before' => '|numeric|between:0,4'
            ],[
                'between' => 'công nhập vào phải lớn hơn :min và nhỏ hơn :max'
            ]);
        }
    ...
    public function updatedOptionAfterNoon(){
        ...
        foreach ($this->option_after_noon as $key => $option)
            $callback = function (Validator $validator) {
                foreach ($this->option_after_noon as $key => $option)
                    $pass = $validator->passes();
                if (!$pass){
                    $this->option_after_noon[$key]['time_after'] = '';
                }
            };
        if ($option['time_after'] != null){
            $this->withValidatorCallback = $callback;
        ...
        $callback_count_after = function (Validator $validator) {
            foreach ($this->option_after_noon as $key => $option)
                $pass = $validator->passes();
            if (!$pass){
                $this->option_after_noon[$key]['count_after'] = '';
            }
        };
        if ($this->input_noon == null && $this->output_noon == null && $this->count_noon == null &&
            ...
            $this->validate([
                ...
                'count_noon' => 'required',
//                'option_after_noon.*.time_after' => 'required',

            ]);
        }
        if ($this->input_noon != null && $this->output_noon != null && $this->count_noon != null && $option['time_after'] != null ){
            $this->withValidatorCallback = $callback_count_after;
            $this->validate([
                'option_after_noon.*.count_after' => '|numeric|between:0,3'
            ],[
                'between' => 'công nhập vào phải lớn hơn :min và nhỏ hơn :max'
            ]);
        }
    ...
    public function updatedOptionBeforeNoon(){
        foreach ($this->option_before_noon as $key => $option)
            $callback = function (Validator $validator) {
                foreach ($this->option_before_noon as $key => $option)
                    $pass = $validator->passes();
                if (!$pass){
                    $this->option_before_noon[$key]['time_before'] = '';
                }
            };
        ...
        if ($option['time_before'] != null){
            $this->withValidatorCallback = $callback;
            $this->validate([
                'option_before_noon.*.time_before' => 'date_format:"H:i"|after:input_noon|before:output_noon',
            ],[
...
//                'before' => 'thời gian nhập phải trước giờ tan  làm ca chiều'
        ...
        $callback_count_before = function (Validator $validator) {
            foreach ($this->option_before_noon as $key => $option)
                $pass = $validator->passes();
            if (!$pass){
                $this->option_before_noon[$key]['count_before'] = '';
            }
        };

        if ($this->input_noon == null && $this->output_noon == null && $this->count_noon == null &&
            $option['time_before'] != null || $option['count_before'] != null && $option['time_before'] == null) {
            $this->withValidatorCallback = $callback_count_before;
            $this->validate([
                'input_noon' => 'required',
                'output_noon' => 'required',
                'count_noon' => 'required',
//                'option_before_noon.*.time_before' => 'required',
//                'option_after_noon.*.time_after' => 'required',
                'option_after_noon.*.count_after' => 'required',
            ]);
        }

        if ($this->input_noon != null && $this->output_noon != null && $this->count_noon != null && $option['time_before'] != null ){
            $this->withValidatorCallback = $callback_count_before;
            $this->validate([
                'option_before_noon.*.count_before' => '|numeric|between:0,3'
            ],[
                'between' => 'công nhập vào phải lớn hơn :min và nhỏ hơn :max'
            ]);
        }
        ...
        foreach ($this->option_after_shift as $key => $option)
            $callback = function (Validator $validator) {
                foreach ($this->option_after_shift as $key => $option)
                    $pass = $validator->passes();
                if (!$pass){
                    $this->option_after_shift[$key]['time_after'] = '';
                }
            };
...
//                'after' => 'thời gian nhập phải sau giờ vào làm ca ',
        ...
        $callback_count_after = function (Validator $validator) {
            foreach ($this->option_after_shift as $key => $option)
                $pass = $validator->passes();
            if (!$pass){
                $this->option_after_shift[$key]['count_after'] = '';
            }
        };
...
//                'option_after_shift.*.time_after' => 'required'
            ]);
        }
        if ($this->input_shift != null && $this->output_shift != null && $this->count_shift != null && $option['time_after'] != null ){
            $this->withValidatorCallback = $callback_count_after;
            $this->validate([
                'option_after_shift.*.count_after' => '|numeric|between:0,3'
            ],[
                'between' => 'công nhập vào phải lớn hơn :min và nhỏ hơn :max'
            ]);
        }
        ...
        foreach ($this->option_before_shift as $key => $option)
            $callback = function (Validator $validator) {
                foreach ($this->option_before_shift as $key => $option)
                    $pass = $validator->passes();
                if (!$pass){
                    $this->option_before_shift[$key]['time_before'] = '';
                }
            };
...
//                'before' => 'thời gian nhập phải trước giờ tan ca '
        ...
        $callback_count_before = function (Validator $validator) {
            foreach ($this->option_before_shift as $key => $option)
                $pass = $validator->passes();
            if (!$pass){
                $this->option_before_shift[$key]['count_before'] = '';
            }
        };
        if ($this->input_shift == null && $this->output_shift == null && $this->count_shift == null &&
            $option['time_before'] != null || $option['count_before'] != null && $option['time_before'] == null) {
            ...
            $this->validate([
                ...
                'output_shift' => 'required',
                'count_shift' => 'required',
//                'option_before_shift.*.time_before' => 'required',
//                'option_after_shift.*.time_after' => 'required',
                'option_after_shift.*.count_after' => 'required',
            ]);
        }
        if ($this->input_shift != null && $this->output_shift != null && $this->count_shift != null && $option['time_before'] != null ){
            $this->withValidatorCallback = $callback_count_before;
            $this->validate([
                'option_before_shift.*.count_before' => '|numeric|between:0,3'
            ],[
                'between' => 'công nhập vào phải lớn hơn :min và nhỏ hơn :max'
            ]);
        }
    ...
    public function store()
    {
        ...
        if($this->type == 1){
            $this->validate([
                'name' => 'required',
                //Sáng
                'input_morning' => 'required|date_format:"H:i"|after:"05:00"|before:"12:01"|',
                'output_morning' => 'required|date_format:"H:i"|after:input_morning|before:"12:01"',
                'count_morning' => 'required|max:4|min:0',
//                'option_after_morning.*.time_after'=>'date_format:"H:i"|after:input_morning|before:output_morning',
                'option_after_morning.*.count_after'=>'max:count_morning',
//                'option_before_morning.*.time_before'=>'date_format:"H:i"|after:input_morning|before:output_morning',
                'option_before_morning.*.count_before'=>'numeric|max:count_morning',

                //Chiều
                'input_noon' => 'required|date_format:"H:i"|after:"12:01"|before:"23:59"|',
                'output_noon' => 'required|date_format:"H:i"|after:input_noon|before:"23:59"',
                'count_noon' => 'required',
//                'option_after_noon.*.time_after'=>'date_format:"H:i"|after:input_noon|before:output_noon',
                'option_after_noon.*.count_after'=>'numeric|max:count_noon',
//                'option_before_noon.*.time_before'=>'date_format:"H:i"|after:input_noon|before:output_noon',
                'option_before_noon.*.count_before'=>'numeric|max:count_noon',
            ]);
        }elseif ($this->type == 2){
...
//dd($data);
    ...
    public function render()
    {
		...
		lForm()->pushBreadcrumb(route("time-keep.timekeeprules.create"),"Create");
        ...
        return view("time-keep::livewire.timekeeprules.create",)->with([
            'day_apply1' => $day_apply1,
        ])
            ->layout('time-keep::layouts.master', ['title' => 'Timekeeprules Create']);
    }
}
