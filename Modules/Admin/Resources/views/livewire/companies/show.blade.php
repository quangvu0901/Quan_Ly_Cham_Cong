<div class="w-full p-2 md:p-4 max-w-lg">
    <x-lf.card class="success" title="Show">
        ...
        <x-slot name="tools">
            @can("admin.companies.listing")
                <a class="btn-primary xs" href="{{route("admin.companies")}}">{!! lfIcon("list",11) !!}</a>
            @endcan
            @can("admin.companies.edit")
                <a class="btn-warning xs" href="{{route("admin.companies.edit",$data->id)}}">{!! lfIcon("edit",11) !!}</a>
            @endcan
        </x-slot>
        <x-slot name="footer">
            <div class="card-footer flex justify-between">
                @can("admin.companies.listing")
                    <a class="btn-primary" href="{{route("admin.companies")}}">{!! lfIcon("list") !!} <span>Listing</span></a>
                @endcan
                <div>
                    @can("admin.companies.edit")
                        <a class="btn-warning" href="{{route("admin.companies.edit",$data->id)}}">{!! lfIcon("edit") !!} <span>Edit</span></a>
                    @endcan
                </div>
            </div>
        </x-slot>
